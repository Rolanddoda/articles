...
By reading this post, you will learn how to add SVG icons from Figma, using and customizing them with a single component:
...
*In `colorBind()` we say: Use the color prop, if it is falsy, then use the default one. Finally, add support for CSS variable color by returning the result of `getVarOrColorValue`.*
*`getVarOrColorValue`: if it starts with `var:` then convert it to a CSS variable. e.g. `var:primary` will return `var(--primary)`. Otherwise will return the string as-is.*
...
export default {
  ...
  methods: {
    getVarOrColorValue(str) {
      return str.startsWith('var:') ? str.replace(/^var:/, 'var(--') + ')' : str
    }
  }
}
</script>
